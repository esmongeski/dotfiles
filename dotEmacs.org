...
First make tab cycling work in normal mode, before evil is loaded.
...
Ok, maybe this should have just been =org-roam-dailies-goto-today=, but checking if the buffer exists is nice to just drop me into the buffer instead of prompting for the header I want to navigate to.
#+begin_src emacs-lisp
  (defun es/open-roam-daily-today ()
    "Thin wrapper around org-roam-dailies-goto-today"
  ...
  (defun es/open-daily-buffer ()
    ...
    (interactive)
    (es/set-org-agenda-files)
    (let ((journal-file (concat "~/org/roam/daily/" (concat (format-time-string "%Y-%m-%d") ".org"))))
      (if
          (not (file-exists-p journal-file))
          (es/open-roam-daily-today) ;;(save-buffer))
