...
assignment update and a DONE transition can both be pushed."
  ...
  (defun es/org-todoist-queue-refile-move (id)
    "Queue a Todoist move for refiled task ID when it remains in todoist.org."
    (when (and id
      ...
      (with-current-buffer (find-file-noselect org-todoist-file)
        (if (es/org-todoist-goto-task-id id)
          ...
          (message "org-todoist: refiled task is no longer in todoist.org; no Todoist move queued")))))
...
#+begin_src emacs-lisp
  ...
  (defun es/org-todoist-build-id-cache (ast)
    ...
    (let ((cache (make-hash-table :test 'equal)))
      (org-element-map ast 'headline
        (lambda (hl)
          (when-let* ((headline (es/org-todoist-headline-node-p hl))
            ...
            (unless (es/org-todoist-id-cache-default-id-p id)
              (let ((type (org-todoist--get-todoist-type hl t)))
                (puthash (cons type id) hl cache)
                (puthash (cons nil id) hl cache))))))
      cache))
  ...
  (defun es/org-todoist-get-by-id-cached (orig type id ast)
    ...
    (if (not (listp ast))
        (progn
          (message "org-todoist get-by-id skipped non-list AST for type=%S id=%S ast=%S"
                   type id ast)
          nil)
      (if-let* ((active es/org-todoist-id-cache-active)
          ...
          node
        (let ((node (condition-case err
                        (funcall orig type id ast)
                      (error
                       (message "org-todoist get-by-id upstream failed for type=%S id=%S: %S"
                                type id err)
                       nil))))
          (if (or (null node)
                  (es/org-todoist-headline-node-p node))
              (progn
                ...
                node)
            (message "org-todoist get-by-id ignored non-headline result for type=%S id=%S result=%S"
                     type id node)
            (when-let ((manual-node (es/org-todoist-get-by-id-manual type id ast)))
              (when-let* ((cache es/org-todoist-id-cache)
                          (key (es/org-todoist-id-cache-key type id)))
                (puthash key manual-node cache)
                (puthash (cons nil id) manual-node cache))
              manual-node))))))
...
Do not refile synced Todoist headings into arbitrary Org files. Wrangle them
with agenda views and org-super-agenda instead.
...
- Keep =org-todoist-delete-remote-items= disabled unless deliberately testing
  deletion behavior.
- Do not use =org-refile= to move synced Todoist headings out of =todoist.org=.
