...
** TODO how do I make this only grab the last ~month? of journal files?
one way - just move stuff out into an archive directory
make it smart enough to match "YYYY-MM" and "YYYY-MM-1"
could also make it MM-1 only before the 15th
oh I should remove all my roam nodes too
#+BEGIN_SRC emacs-lisp
    ...
    (custom-set-variables
     '(org-agenda-files
       (cl-remove-if (lambda (k) (string-match-p "archive.org" k))
                     (append (find-lisp-find-files "~/Dropbox/org/projects/" "\.org$")
