...
`:raw-value' and a list in `:title'."
  ...
  (defun es/org-todoist-task-in-target-location-p (task project section)
    "Return non-nil when TASK is already under target PROJECT and SECTION.
Todoist reuses the synthetic default section id across every project. Upstream
only compares section ids, so moving an item from one project's default section
to another project's default section can be missed. Compare project position as
well."
    (and project
         section
         (cl-equalp (org-todoist--get-project-id-position task)
                    (org-todoist--id-or-temp-id project))
         (cl-equalp (org-todoist--get-section-id-position task)
                    (org-todoist--id-or-temp-id section))))

  (defun es/org-todoist-update-tasks (tasks ast)
    "Update all tasks in AST using TASKS, with default-section move fixes."
    (cl-loop for data across tasks do
             (let* ((id (assoc-default 'id data))
                    (proj (org-todoist--get-by-id org-todoist--project-type
                                                  (assoc-default 'project_id data)
                                                  ast))
                    (section-id (assoc-default 'section_id data))
                    (section (if section-id
                                 (org-todoist--get-by-id org-todoist--section-type
                                                         section-id proj)
                               (org-todoist--get-by-id org-todoist--section-type
                                                       org-todoist--default-id
                                                       proj)))
                    ...
                    (task (if deleted
                            ...
                            (org-todoist--get-or-create-node
                             section org-todoist--task-type id title description data
                             org-todoist--task-skip-list ast)))
               ...
               (when task
                 (if deleted
                   ...
                   (unless (or section parent-id)
                     (setq section
                           (org-todoist--get-by-id org-todoist--section-type
                                                   ...
                                                   proj)))
                   (unless (or parent-id
                               (es/org-todoist-task-in-target-location-p
                                task proj section))
                   ...
                   (org-todoist--set-todo task
                                          (assoc-default 'checked data)
                                          (assoc-default 'is_deleted data))))))
    (let ((with-child nil))
      (cl-loop for data across tasks do
               (when-let* ((parenttaskid (assoc-default 'parent_id data))
                           (task (org-todoist--get-by-id
                                  org-todoist--task-type
                                  (assoc-default 'id data)
                                  ast))
                           (parenttask (org-todoist--get-by-id
                                        org-todoist--task-type parenttaskid ast))
                           (wrong-spot
                            (not (cl-equalp parenttaskid
                                            (org-todoist--get-task-id-position
                                             task)))))
                 (push parenttask with-child)
                 (org-todoist--adopt-sub parenttask task)))))

  (defun es/org-todoist-set-todo (node checked &optional deleted)
    "Set NODE TODO state from Todoist CHECKED and DELETED values.
Upstream changes `:todo-type' for unchecked tasks but leaves an existing
`DONE' keyword in place. That creates a visibly DONE heading for a live
Todoist task. Always set the keyword to match the remote state."
    (cond
     ((eql t deleted)
...
public API."
    ...
    (defalias 'org-todoist--update-tasks #'es/org-todoist-update-tasks)
    (defalias 'org-todoist--set-todo #'es/org-todoist-set-todo)
...
- =:otdfs= runs a full Todoist refresh using the Todoist full-sync token and
  prunes task headings that are no longer present in the fetched remote payload.
