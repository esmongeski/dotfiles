          ...
          '(org-agenda-files
            (append
             (cl-remove-if (lambda (k) (string-match-p "archive.org" k))
                           (find-lisp-find-files (concat es/org-root "/projects/") "\.org$"))
             (cl-remove-if (lambda (k) (string-match-p "_done.org" k))
                           (find-lisp-find-files (concat es/org-work-root "/projects/") "\.org$"))
             (cl-remove-if (lambda (k) (not (or
                                             (string-match-p (es/current-month) k)
                                             (string-match-p (es/fourteen-days-ago-month) k))))
                           (find-lisp-find-files (concat es/org-work-root "/roam/daily") "\.org$"))
             (cl-remove-if (lambda (k) (not (string-match-p "inbox.org" k)))
                           (find-lisp-find-files es/org-root "\.org$"))
             (cl-remove-if (lambda (k) (not (or
                                             (string-match-p (es/current-month) k)
                                             (string-match-p (es/fourteen-days-ago-month) k))))
                           (find-lisp-find-files (concat es/org-root "/roam/daily/") "\.org$"))
             )
            )))
...
#+begin_src emacs-lisp
  ...
  ;;(define-key evil-ex-map "osp" 'org-store-link)
...
#+begin_src emacs-lisp
  ...
  ;;(defun todoist--get-tasks-to-me ()
  ;;  "Get the list of all tasks."
  ;;  (append (todoist--query "GET" "/tasks" (make-hash-table :labels ("to_me"))) nil))

#+end_src
  what do I really want?
  move tasks that are assigned to me into agenda
  so they need to go in a personal file somewhere, that's fine
  keep the todoist metadata so I can toggle todoist mode and mark them done from emacs

  i probably want a work project in todoist so pull that down too
...
#+begin_src emacs-lisp
  ...
  (defun todoist--insert-today-assigned-to-me (tasks)
    "Insert today's tasks that are assigned to me.

  TASKS is the list of tasks."
    (mapcar (lambda (task) (todoist--insert-task task 2 t)) (-filter 'todoist--is-today-and-me tasks)))
  ...
  (defun todoist--is-me (task)
      (equal (todoist--task-asignee task) "4330893")
    )
  ...
  TASKS is the list of tasks."
    (mapcar (lambda (task) (todoist--insert-task task 2 t)) (-filter 'todoist--is-me tasks)))
    ...
    #+begin_src emacs-lisp
      ...
      (defun todoist--insert-work (tasks)
        "Insert today's tasks that are assigned to me.
                ...
                TASKS is the list of tasks."
        (mapcar (lambda (task) (es/todoist--insert-task task 2 t)) (-filter 'todoist--is-work tasks)))
