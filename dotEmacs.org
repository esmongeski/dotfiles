...
#+begin_src emacs-lisp
  ...
  (defun es/org-todoist-get-by-id-cached (orig type id ast)
    ...
    (if-let* ((active es/org-todoist-id-cache-active)
        ...
        node
      (let ((node (funcall orig type id ast)))
...
#+begin_src emacs-lisp
  ...
  (unless (require 'org-todoist nil t)
...
** general
#+begin_src emacs-lisp
;; org-todoist handles sync and completion from the synced file.
;; First-pass migration intentionally leaves the old ex commands disabled.
