...
#+begin_src emacs-lisp
  (setq org-roam-head
        (concat "#+title: %<%Y-%m-%d (%A)>\n#+startup: showall\n"
                "* Do Today\n* Journal\n"))
  (setq personal-header
        (concat "Personal - " org-roam-head))
  (setq work-header
        (concat "Work - " org-roam-head))
  ...
  (setq org-roam-dailies-capture-templates
        (let ((head
          ...
          `(("j" "journal" entry
             "* %<%H:%M> %?"
             :if-new (file+head+olp ,personal-file ,head ("Journal")))
            ("t" "do today - work" entry
             "** TODO [#A] %a\nSCHEDULED: <%<%Y-%m-%d>>"
             :if-new (file+head+olp ,work-file ,head ("Do Today"))
             :immediate-finish t)
            ("p" "do today - personal" entry
             "** TODO [#A] %a\nSCHEDULED: <%<%Y-%m-%d>>"
             :if-new (file+head+olp ,personal-file ,head ("Do Today"))
             :immediate-finish t)
            )))
