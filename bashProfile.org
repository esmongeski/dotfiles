...
#+begin_src bash
  ...
  function _update_ps1() {
      PS1="$($GOPATH/bin/powerline-go -error $? -jobs $(jobs -p | wc -l) -shell bash -cwd-max-dir-size 2 -colorize-hostname -hostname-only-if-ssh -path-aliases=$EXTRA_POWERLINE_DIR_ALIASES -truncate-segment-width 5 $EXTRA_POWERLINE_OPTIONS)"
